...
import (
	...
	memkeystore "github.com/yanhuangpai/voyager/pkg/keystore/mem"
...
<*)))))>< `
			...
			b, c, err := node.NewVoyager(c.config.GetString(optionNameP2PAddr), signerConfig.address, *signerConfig.publicKey, signerConfig.signer, c.config.GetUint64(optionNameNetworkID), logger, signerConfig.libp2pPrivateKey, signerConfig.pssPrivateKey, node.Options{
			...
			if isWindowsService {
				s, err := service.New(p, &service.Config{
					Name:        serviceName,
					DisplayName: "Voyager",
					Description: "Voyager, Infinity client.",
				})
				if err != nil {
					return err
				}
...
func (c *command) configureSigner(cmd *cobra.Command, logger logging.Logger) (config *signerConfig, err error) {
	...
	if c.config.GetBool(optionNameClefSignerEnable) {
		...
		logger.Infof("using infinity network address through clef: %s", address)
	} else {
		logger.Warning("clef is not enabled; portability and security of your keys is sub optimal")
		infinityPrivateKey, created, err := keystore.Key("infinity", password)
		if err != nil {
			return nil, fmt.Errorf("infinity key: %w", err)
		}
		...
		if created {
			logger.Infof("new infinity network address created: %s", address)
		} else {
			logger.Infof("using existing infinity network address: %s", address)
		}
	}

	logger.Infof("infinity public key %x", crypto.EncodeSecp256k1PublicKey(publicKey))
	...
	logger.Infof("using Infinity address %x", overlayEthAddress)
